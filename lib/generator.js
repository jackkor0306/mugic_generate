...
function callClaude(prompt, opts) {
  ...
  return new Promise((resolve, reject) => {
    const args = ['-p', '--output-format', 'stream-json', '--include-partial-messages', '--verbose', '--model', MODEL];
    if (opts.allowRead) args.push('--allowedTools', 'Read');
    const env = { ...process.env };
    if (opts.maxThinkingTokens) env.MAX_THINKING_TOKENS = String(opts.maxThinkingTokens);
    ...
    // 행(hang) 감지: 5분간 아무 데이터도 오지 않으면 중단
    let lastData = Date.now();
    const idleTimer = setInterval(() => {
      if (Date.now() - lastData > 5 * 60 * 1000) {
        clearInterval(idleTimer);
        child.kill();
        reject(new Error('AI 응답이 5분간 없어 작업을 중단했습니다. 다시 시도해 주세요.'));
      }
    }, 30000);
    ...
    const report = () => {
      ...
      lastReport = now;
      if (textChars > 0) onProgress(`결과 작성 중... (${textChars.toLocaleString()}자)`);
      else if (thinkChars > 0) onProgress(`구상 중... (사고 ${thinkChars.toLocaleString()}자)`);
      else onProgress('시작하는 중...');
    };
    ...
    child.stdout.on('data', (d) => {
      ...
      while ((idx = lineBuf.indexOf('\n')) >= 0) {
        ...
        if (ev.type === 'stream_event' && ev.event && ev.event.type === 'content_block_delta' && ev.event.delta) {
          ...
          if (typeof delta.thinking === 'string') thinkChars += delta.thinking.length;
          else if (typeof delta.text === 'string') textChars += delta.text.length;
          else if (typeof delta.data === 'string') thinkChars += delta.data.length; // 암호화된 사고 델타
          else thinkChars += 1; // 그 외 델타도 활동으로 집계
          report();
        } else if (ev.type === 'rate_limit_event') {
          if (thinkChars === 0 && textChars === 0) {
            lastReport = 0;
            onProgress('AI 사용량 한도 확인 중... 한도에 걸린 경우 자동으로 대기 후 재개됩니다.');
          }
    ...
    child.stderr.on('data', (d) => { lastData = Date.now(); stderr += d; });
    child.on('error', (err) => { clearTimeout(timer); clearInterval(idleTimer); reject(err); });
    child.on('close', () => {
      clearTimeout(timer);
      clearInterval(idleTimer);
      if (resultText === null) {
        return reject(new Error('Claude 응답을 받지 못했습니다: ' + stderr.slice(0, 500)));
      }
      if (resultIsError) return reject(new Error('Claude 오류: ' + resultText.slice(0, 500)));
      resolve(resultText);
    });
...
function scorePrompt(song, feedback, prevScore) {
  ...
  lines.push('   - 코드 심볼은 실용음악(재즈/팝) 표기법으로 상세하게 표기할 것:');
  lines.push('     · 7th/메이저7th(Am7, Fmaj7, G7), 텐션(C9, Dm11, G13, Cadd9), sus(Gsus4), dim/하프디미니시(Bm7b5), aug 등을 화성에 맞게 적극 사용.');
  lines.push('     · 분수코드(온코드) 표기 필수: 베이스가 3음/5음일 때 C/E, F/C 처럼 표기.');
  lines.push('     · 화성 리듬: 마디당 1~2회 이상 코드 체인지를 기본으로 하고, 투파이브(ii-V), 세컨더리 도미넌트, 경과 코드를 활용해 진행을 세련되게.');
  lines.push('     · 표기한 코드와 반주 보이싱(실제 음)이 반드시 일치할 것. 단순 3화음(C, Am, F, G)만 반복하는 표기는 금지.');
  lines.push('     · 코드 심볼은 화성이 바뀌는 시점의 음표 앞에 "Dm7" 형식으로 붙일 것 (주로 메인 반주 보이스에).');
  ...
  lines.push('4. 협화 규칙 (반드시 검증): 같은 순간에 함께 울리는 보컬 멜로디 음과 반주 성부의 음이 단2도·장2도(반음/온음) 간격으로 충돌하지 않도록 할 것. 단9도 충돌도 피할 것. 충돌이 생기면 반주 쪽 음을 코드 내 다른 음으로 바꾸거나 전위/생략으로 회피할 것. 경과음 등 비화성음은 약박에서 짧게만 사용. 작곡하면서 확인하되, 별도의 장황한 재검토 과정 없이 한 번에 정확히 쓸 것.');
...
async function runJob(jobId, song, type, feedback) {
  try {
    ...
    const updateProgress = (msg) => {
      try { db.prepare('UPDATE jobs SET progress = ? WHERE id = ?').run(msg, jobId); } catch (e) {}
    };
    // 사고량 제한: 무제한 사고로 작곡이 20분+ 걸리는 것을 방지
    const claudeOpts = {
      allowRead: attach.hasImages,
      onProgress: updateProgress,
      maxThinkingTokens: type === 'score' ? 12000 : 4000,
    };
    if (type === 'lyrics') {
