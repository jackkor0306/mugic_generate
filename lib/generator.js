    ...
    child.stdout.on('data', (d) => {
      ...
      while ((idx = lineBuf.indexOf('\n')) >= 0) {
        ...
        if (ev.type === 'stream_event' && ev.event && ev.event.type === 'content_block_delta' && ev.event.delta) {
          if (typeof ev.event.delta.thinking === 'string') thinkChars += ev.event.delta.thinking.length;
          if (typeof ev.event.delta.text === 'string') textChars += ev.event.delta.text.length;
          report();
...
async function runJob(jobId, song, type, feedback) {
  try {
    ...
    const claudeOpts = { allowRead: attach.hasImages, onProgress: updateProgress };
    if (type === 'lyrics') {
